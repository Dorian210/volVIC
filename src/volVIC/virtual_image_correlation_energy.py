...
class VirtualImageCorrelationEnergyElem:
    ...
    def make_intg_space(
        ...
        (xi, eta), (dxi, deta) = linspace_for_VIC_elem(
            self.spline, self.ctrl_pts, dist=surf_dx, alpha=padding
        )
        n = int(np.rint(2 * h / width_dx))
        true_dx = 2 * h / n
        gamma = np.linspace(-h + 0.5 * true_dx, h - 0.5 * true_dx, n)
        gamma = np.insert(gamma, [n // 2, -(n - 1) // 2], [-eps, eps])
    ...
    def make_operators(
        ...
        phi1_p = sps.hstack(
            (
                dN_deta.multiply(A3_over_Se[0, :, None]),
        ...
        phi2_p = sps.hstack(
            (
                -dN_dxi.multiply(A3_over_Se[0, :, None]),
        ...
        intg_weights = np.kron(
            np.kron(self.dxi, self.deta), self.dgamma
        )  # Shape: (n_points,)
    ...
    def f_df_dX(
        ...
        (df_dx, df_dy, df_dz), f = TriLinearRegularGridInterpolator().grad(
...
#             u + eps * dx_u, rho + eps * dx_rho, image
#         )
...
#     h = size / 8
#     energy = VirtualImageCorrelationEnergyElem(
#         spline, ctrl_pts, surf_dx=1.0, alpha=0.0, width_dx=1.0, h=h
#     )
...
#     plotter = pv.Plotter()
#     plotter.add_mesh(
#         pv.PolyData(energy.Xv0.reshape((3, -1)).T),
#         color="green",
#         point_size=5,
#         render_points_as_spheres=True,
#     )
#     plotter.add_mesh(
#         pv.PolyData(Xv.reshape((3, -1)).T),
#         color="blue",
#         point_size=5,
#         render_points_as_spheres=True,
#     )
...
def make_image_energies(
    ...
    if isinstance(mesh, MeshLattice):
        ...
        image_energies_cell = parallel_blocks(
            VirtualImageCorrelationEnergyElem,
            args_list,
            verbose=verbose,
            pbar_title="Init image energies",
            ...
            for j in range(mesh.m):
                for k in range(mesh.n):
                    for cell_patch in range(cell_mesh.connectivity.nb_patchs):
                        patch = np.ravel_multi_index(
                            (i, j, k, cell_patch),
                            (mesh.l, mesh.m, mesh.n, cell_mesh.connectivity.nb_patchs),
                        )
                        image_energies.append(
                            image_energies_cell[cell_patch].rigid_body_copy(
                                separated_ctrl_pts[patch]
                            )
                        )
    else:
        ...
        image_energies = parallel_blocks(
            VirtualImageCorrelationEnergyElem,
            args_list,
            verbose=verbose,
            pbar_title="Init image energies",
...
def plot_last_profile(
    ...
    mean_inner = lambda e, field: (
        field.reshape((e.xi.size * e.eta.size, e.gamma.size))
        * e.wdetJs.reshape((-1, 1))
    ).sum(axis=0)
    compute_surf_means = (
        lambda field_name: np.array(
            [mean_inner(e, e.saved_data[field_name]) for e in image_energies]
        )
        / areas[:, None]
    )
    var_inner = lambda e, field, mean: (
        (field.reshape((e.xi.size * e.eta.size, e.gamma.size)) - mean[None, :]) ** 2
        * e.wdetJs.reshape((-1, 1))
    ).sum(axis=0)
    compute_surf_vars = (
        lambda field_name, means: np.array(
            [
                var_inner(e, e.saved_data[field_name], m)
                for e, m in zip(image_energies, means)
            ]
        )
        / areas[:, None]
    )
    ...
    r_std_inter = np.sqrt(
        ((r_means - r_mean[None, :]) ** 2 / areas[:, None]).sum(axis=0)
    )
    f_std_inter = np.sqrt(
        ((f_means - f_mean[None, :]) ** 2 / areas[:, None]).sum(axis=0)
    )
    ...
    ax1.fill_between(
        gamma, f_mean - f_std_inter, f_mean + f_std_inter, alpha=0.3, color="#d95f02"
    )
    ...
    ax2.fill_between(
        gamma, f_mean - f_std_intra, f_mean + f_std_intra, alpha=0.3, color="#d95f02"
    )
...
def compute_image_energy_operators(
    ...
    args_list = [
        (separated_u_field[patch], image_energies[patch], rho)
        for patch in range(mesh.connectivity.nb_patchs)
    ]
    res = parallel_blocks(
        process_patch,
        args_list,
        verbose=verbose,
        pbar_title="Compute image energy",
        shared_mem_last_arg=image,
        disable_parallel=disable_parallel,
    )
...
def compute_distance_field_patch(
    ...
    real_profiles = f.reshape(
        (image_energy.xi.size, image_energy.eta.size, image_energy.gamma.size)
    )
    ...
    for i in range(max_iter):
        ...
        target_profiles = image_energy.virtual_image(
            np.zeros(1), np.zeros(1), gamma_d.ravel(), rho
        )[0].reshape(gamma_d.shape)
...
def compute_distance_field(
    ...
    args_list = [
        (e, sav["last_saved_f"], sav["last_saved_rho"])
        for e, sav in zip(image_energies, saved_data)
    ]
    d = parallel_blocks(
        compute_distance_field_patch,
        args_list,
        verbose=verbose,
        pbar_title="Compute distance field",
        disable_parallel=disable_parallel,
    )

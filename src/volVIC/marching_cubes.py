...
import numba as nb
from numba.typed.typeddict import Dict as nb_Dict
import meshio as io

# tri_table : NDArray of shape (256, 15), dtype=np.uint8
tri_table = np.array(
    [
...
# edge_vertex_offsets : NDArray of shape (12,3), dtype=np.float32
...
@nb.njit(cache=True)
def marching_cubes_nb(volume: NDArray, threshold: float):
    verts = []
    faces = []
    vert_idx = nb_Dict.empty(key_type=key_type, value_type=nb.int64)
            ...
            for x in range(X - 1):
                ...
                for i in range(0, length, 3):
                    ...
                    for edge in edges:
                        ...
                        t = eps if t <= eps else (1.0 - eps) if t >= (1.0 - eps) else t
...
def marching_cubes(volume: NDArray, threshold: float) -> io.Mesh:
    """

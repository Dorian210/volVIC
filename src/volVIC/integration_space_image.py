...
def get_tangent(
    xi, eta, axis, ctrl_pts, p_xi, p_eta, knot_xi, knot_eta, m_xi, m_eta, n_xi, n_eta
):
    xi = np.array([xi])
    eta = np.array([eta])
    if axis == 0:
...
def get_dxi(
    xi, eta, dist, ctrl_pts, p_xi, p_eta, knot_xi, knot_eta, m_xi, m_eta, n_xi, n_eta
):
    dX_dxi = get_tangent(
        xi, eta, 0, ctrl_pts, p_xi, p_eta, knot_xi, knot_eta, m_xi, m_eta, n_xi, n_eta
    )
...
def get_deta(
    ...
    dX_deta = get_tangent(
        xi, eta, 1, ctrl_pts, p_xi, p_eta, knot_xi, knot_eta, m_xi, m_eta, n_xi, n_eta
    )
...
def linspace_for_VIC_elem_numba(
    alpha, p_xi, p_eta, knot_xi, knot_eta, span_xi, span_eta, ctrl_pts, dx
):
